...
def compute_hourly_salary(
    ...
    時薪制薪資試算（Think4U 規則）：
      ...
      投保薪資 = 工時薪資（以當月時薪×工時估算月投保薪資，再對級距表）
    ...
    # 估算月投保薪資 = 工時薪資（對級距表）
    insured_base = work_pay
    labor = labor_insurance_employee(insured_base) if work_pay > 0 else 0
    health = health_insurance_employee(insured_base, dependents) if work_pay > 0 else 0
    ...
    return {
        ...
        "net": net,
        "daily": rate,              # 顯示用：時薪
        "labor_grade": _grade(insured_base, LABOR_GRADES) if work_pay > 0 else 0,
        "health_grade": _grade(insured_base, HEALTH_GRADES) if work_pay > 0 else 0,
    }
